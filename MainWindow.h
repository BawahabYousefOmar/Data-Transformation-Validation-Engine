...
// Forward declarations (widget classes defined in their own headers)
class UploadWidget;
...
// ============================================================
//  BatchRecord
//  Carries the result of one processed batch so the Reports
//  table can display it and open a details window.
// ============================================================
struct BatchRecord {
...
//
//  Responsibilities:
//    - Application shell: sidebar navigation, page switching
//    - Role-gating (admin vs worker) — hides Standards nav item
//    - Owns the pipeline: receives filesReadyForValidation(),
//      runs HeaderProcessor → RecordParser → Validator, stores result
//    - Emits result to ReportsTableWidget for display
//    - Launches NoteDialog and ReportDetailsWindow on demand
// ============================================================
...
private:
    ...
    // In-memory store of processed batches (keyed by batchId)
    QMap<QString, BatchRecord> m_batches;

    // Shared backend objects (reloaded once at startup)
