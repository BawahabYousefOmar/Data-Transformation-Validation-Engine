...
// Yousef's widget (forward declared — include when his header is ready)
...
// ----------------------------------------------------------------
// onRunPipeline — the core validation pipeline
// Called by UploadWidget when user confirms a batch name.
// ----------------------------------------------------------------
...
void MainWindow::onOpenDetails(const QString& batchId)
{
    ...
    // ReportDetailsWindow is Yousef's responsibility.
    // Amro instantiates it here and passes the batch data.
    // #include "ReportDetailsWindow.h"
    // auto* win = new ReportDetailsWindow(m_batches[batchId], this);
...
{
    // Hide main window, re-show login (handled by main.cpp via close event)
    close();
}
                                               ...
                                               const QString& batchId)
{
    ...
    // ── 1. Build parser with unknown-header callback ─────────
    ...
    // ── 2. Feed each file through the parser ─────────────────
