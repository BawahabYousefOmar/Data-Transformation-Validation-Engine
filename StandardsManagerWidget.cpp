    ...
    , m_ruleLoader(ruleLoader)
{
    ...
    // Security check: Disable UI if not admin (though MainWindow should hide it)
    if (!m_isAdmin) {
...
void StandardsManagerWidget::populateTable()
{
    ...
    // The standard fields DataValve supports
    ...
    for (const QString& field : standardFields) {
        ...
        // 2. Mapped Aliases (Mocked string for UI - requires a reverse lookup in HeaderProcessor)
        // For now, we put a placeholder. You can update this to pull from m_headerProc.
...
void StandardsManagerWidget::openRuleEditor(const QString& fieldName)
{
    ...
    if (dlg.exec() == QDialog::Accepted) {
        ...
        // Optional: Call a saveToFile() method on RuleLoader here to persist it to rules.txt
    }
}
